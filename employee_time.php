        ...
        if (mysqli_num_rows($attendance_result) == 0) {
            ...
            if ($inorout == "OUT") {
                if (!is_null($attendance_row['clock_out_time'])) {
                    ...
                    if ($clock_in_datetime < $morning_end) {
                        $morning_hours = min($clock_out_datetime, $morning_end) - max($clock_in_datetime, $morning_start);
                        $morning_hours = max($morning_hours / 3600, 0);
                    }

                    // Afternoon hours (1:00 PM - 5:00 PM)
                    if ($clock_out_datetime > $afternoon_start) {
                        $afternoon_hours = min($clock_out_datetime, $afternoon_end) - max($clock_in_datetime, $afternoon_start);
                        $afternoon_hours = max($afternoon_hours / 3600, 0);
                    }

                    // Overtime hours (after 5:30 PM)
                    ...
                    // Determine status based on total hours and time
                    if ($clock_out_datetime >= $overtime_start) {
                        $status = "Over Time";
